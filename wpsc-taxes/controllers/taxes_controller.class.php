...
class wpec_taxes_controller {
	...
	function wpec_taxes_calculate_total() {
		...
		if ( $this->wpec_taxes->wpec_taxes_get_enabled() ) {
			...
			if ( $this->wpec_taxes_run_logic() ) {
				...
				$apply_coupons_after_taxes = false;
				if ( $coupon_num ) {
					$coupons = new wpsc_coupons( $coupon_num );
					$free_shipping = $coupons->is_free_shipping();
					$apply_coupons_after_taxes = $coupons->after_taxes();
				}

				// minus coupon tax if we are using coupons, and the coupon is 
				// not a free shipping coupon, and the coupon isn't an after-tax
				// coupon.

				// @TODO: Make sure this doesn't pop up again.
	...
	* */
	function wpsc_build_taxes_row( $row_mode = 'rates', $row_key = 0, $tax_rate = false ) {
		...
		ob_start();
		?>
		<tr id='wpsc-taxes-<?php echo esc_attr( $row_mode ); ?>-row-<?php echo esc_attr( $row_key ); ?>' data-row-key="<?php echo esc_attr( $row_key ); ?>" class='wpsc-tax-<?php echo esc_attr( $row_mode ); ?>-row <?php echo esc_attr( $tax_rate['row_class'] ); ?>'>

			<?php if ( $row_mode == 'bands' ) : // BAND NAME ?>
				<td>
					<input type='hidden' id='band-index-<?php echo esc_attr( $row_key ); ?>' name="wpsc_options[wpec_taxes_bands][<?php echo esc_attr( $row_key ); ?>][index]" value="<?php echo esc_attr( $row_key ); ?>" />
					<input id='band-name-<?php echo esc_attr( $row_key ); ?>' name="wpsc_options[wpec_taxes_bands][<?php echo esc_attr( $row_key ); ?>][name]" class='taxes-band' type='text' value='<?php echo esc_attr( $tax_rate['name'] ); ?>' />
				</td>
			<?php endif; ?>
			...
			<td><?php // TAX RATE ?>
				<input type='text' size='3' id="<?php echo esc_attr( $row_mode ); ?>-rate-<?php echo esc_attr( $row_key ); ?>" name="wpsc_options[wpec_taxes_<?php echo esc_attr( $row_mode ); ?>][<?php echo esc_attr( $row_key ); ?>][rate]" class="taxes-<?php esc_attr_e( $row_mode ); ?>" value="<?php echo esc_attr( $tax_rate['rate'] ); ?>" /> %
			</td>

			<?php if ( $row_mode == 'rates' ): // TAX SHIPPING ? ?>
				<td>
					<label>
						<input type='checkbox' id="shipping-<?php echo esc_attr( $row_key ); ?>" name="wpsc_options[wpec_taxes_<?php esc_attr_e( $row_mode ); ?>][<?php echo esc_attr( $row_key ); ?>][shipping]" class="taxes-<?php echo esc_attr( $row_mode ); ?>" <?php checked( $tax_rate['shipping'] == 1 ); ?> />
			...
			<?php // ACTIONS ?>
			<td>
				<a tabindex="-1" title="<?php _e( 'Delete Field', 'wp-e-commerce' ); ?>" class="button-secondary wpsc-button-round wpsc-button-minus wpsc-taxes-<?php echo esc_attr( $row_mode ); ?>-delete" id="wpsc-taxes-<?php echo esc_attr( $row_mode ); ?>-delete-<?php echo esc_attr( $row_key ); ?>" href="#"><?php echo _x( '&ndash;', 'delete item', 'wp-e-commerce' ); ?></a>
				<a tabindex="-1" title="<?php _e( 'Add Field', 'wp-e-commerce' ); ?>" class="button-secondary wpsc-button-round wpsc-button-plus wpsc-taxes-<?php echo esc_attr( $row_mode ); ?>-add" href="#"><?php echo _x( '+', 'add item', 'wp-e-commerce' ); ?></a>
			</td>
